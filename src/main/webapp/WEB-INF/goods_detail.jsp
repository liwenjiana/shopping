...
<div id="shop_detail">
    ...
    <div class="right_detail lf">
        ...
        <p>
            <s>规格：</s>
            <ul style="margin-left: 40px" id="itemGoodsUl">
                  <c:forEach items="${itemGoods}" var="ig">
                         <li  
                          <c:if test="${ig.id==goods.id }">
                                  class="current"
                          </c:if>
                         ><a   href="detail.do?id=${ig.id}">${ig.title }</a></li>
                  </c:forEach>
            </ul>
        </p>
        <!-- 未选择规格，颜色时状态-->
        ...
        <p class="accountChose">
            ...
            <button class="numberMinus">-</button>
	            <input type="text" value="1" class="number" id="buy-num" name="goodsCount">
            <button class="numberAdd">+</button>
              <form id="buyForm">
	            <input type="hidden" value="${goods.id }" name="goodsId">
	            <input type="hidden" value="${goods.image }" name="goodsImage">
	            <input type="hidden" value="${goods.title }" name="goodsTitle">
	            <input type="hidden" value="${goods.itemType }" name="goodsItemType">
	            <input type="hidden" value="${goods.price }" name="goodsPrice">
            </form>
        </p>
        <!-- 购买部分-->
        <div class="shops">
...
<script type="text/javascript">
    ...
    $("#add_cart").click(function (e) {
    	var goodsCount="&goodsCount="+$("#buy-num").val();
    	   var data=$("#buyForm").serialize()+goodsCount;
		    $.ajax({
		    	"url":"../cart/add.do",
		    	"data":data,
		    	"type":"post",
...
<script type="text/javascript">
   $("#itemGoodsUl li").hover(function(){
	   $("#itemGoodsUl li").removeClass("current");
