...
<div class="lf1">
    ...
    <div class="lf1_bottom">
        <div class="item_cat lf">
            ...
            <c:forEach items="${goodsCategories}" var="gc" varStatus="userStatus">
                <p>${gc.name}</p>
                 <ul>
                <c:forEach items="${computerCategory[userStatus.index]}" var="cc">
                           <li><a href="../goods/list.do?category_id=${cc.id}&order_by=0&&page=1">${cc.name}</a></li>
                </c:forEach>  
                </ul>           
                </c:forEach>
            </div>
        </div>
        <c:forEach items="${computers}" var="com">
        <div class="item_msg lf">
            <img src="${pageContext.request.contextPath}${com.image}" alt=""/>

            <p class="bottom_ys2">${com.title}笔记本电脑</p>

            <p class="bottom_ys3">${com.price}</p>

            <p class="bottom_ys4 color_2"><a href="${com.id}">查看详情</a></p>
        </div>
        </c:forEach>
    </div>
</div>
